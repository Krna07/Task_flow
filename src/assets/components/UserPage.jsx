    ...
    <div className="flex md:flex-col justify-between w-full">
      ...
      <ul className="flex md:flex-col gap-2 md:gap-3 mt-4 md:mt-10 w-full">
        <li>
          <button
            onClick={showNoti}
            className={`flex items-center gap-2 md:gap-3 font-semibold px-3 py-2 md:px-4 md:py-2.5 rounded-xl transition-all ${
              isAssignedVisible
                ? 'bg-indigo-600 text-white shadow-lg scale-105'
                : 'text-gray-700 hover:bg-indigo-100 hover:scale-105'
            }`}
          >
            <AssignedTaskIcon />
            ...
            <div className="flex flex-wrap gap-2">
              {['Daily_Task', 'Weekly_Task', 'Emergency_Task', 'Aera-Gaira', 'Others'].map(cat => (
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-full transition-all font-semibold ${
                    category === cat
                      ? 'bg-indigo-600 text-white shadow-inner shadow-indigo-300 scale-105'
          ...
          {assignedTasks.length ? (
            <ul className="space-y-3">
              {assignedTasks.map((task, i) => (
                <li key={i} className="p-3 sm:p-4 bg-gray-50 rounded-xl border shadow-sm hover:shadow-md transition-all">
